...
import { Check, ChevronRight, CircleX, FileText, Quote, UserCheck } from 'lucide-react'
import { api } from '../api'
...
export function ReviewPage() {
  ...
  if (loading) return <Fragment><LoadingState />{toast && <Toast message={toast} />}</Fragment>
  if (error) return <Fragment><ErrorState message={error} retry={load} />{toast && <Toast message={toast} />}</Fragment>
  if (!items.length) return <Fragment><EmptyState title="审核队列已清空" detail="所有待审核知识均已处理" />{toast && <Toast message={toast} />}</Fragment>
  return <div className="review-layout"><aside className="review-queue"><div className="review-caption"><span>等待处理</span><strong>{items.length}</strong></div>{items.map((item) => <button key={item.id} className={selected?.id === item.id ? 'active' : ''} onClick={() => setSelected(item)}><span className="queue-icon"><FileText size={18} /></span><div><strong>{item.title}</strong><small>{item.domain} · {item.createdBy}</small></div><ChevronRight size={16} /></button>)}</aside>{selected && <section className="review-document"><header><div><StatusBadge status={selected.status} /><h2>{selected.title}</h2><p>{selected.domain} · 来源：{selected.source} · 版本 v{selected.version}</p></div><span className="review-safety"><UserCheck size={18} />等待人工审核</span></header><div className="document-meta"><span>创建人<strong>{selected.createdBy}</strong></span><span>切片数量<strong>{selected.chunkCount} 段</strong></span><span>更新时间<strong>{new Date(selected.updatedAt).toLocaleString('zh-CN')}</strong></span><span>标签<strong>{selected.tags.join(' · ') || '无标签'}</strong></span></div><article><Quote size={24} /><p>{selected.content}</p></article><div className="review-decision"><label><span>审核意见</span><textarea rows={3} value={comment} onChange={(event) => setComment(event.target.value)} /></label><div><button className="reject-button" onClick={() => review(false)}><CircleX size={17} />驳回修改</button><button className="primary-button" onClick={() => review(true)}><Check size={17} />通过并生效</button></div></div></section>}{toast && <Toast message={toast} />}</div>
}
