...
- 生产拓扑：PostgreSQL、Kafka、MinIO、Milvus、独立 API/Worker、SiliconFlow Chat/Embedding 与 Authing OIDC，使用 Outbox、任务租约和可恢复 Worker。
...
```mermaid
flowchart LR
    OIDC["Authing OIDC / PKCE"] --> UI["React 运营台"]
...
先注册 Authing 免费版应用并创建 SiliconFlow API Key。生产 Compose 默认调用 SiliconFlow 的 `Qwen/Qwen3-8B` 与 `BAAI/bge-m3`，不会下载本地模型或身份服务。

```bash
cp .env.example .env
# 修改 .env 中的密码、SILICONFLOW_API_KEY 和 Authing 配置
