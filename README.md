...
- 生产拓扑：PostgreSQL、Kafka、MinIO、Milvus、独立 API/Worker、OpenAI-compatible Embedding 与 Chat Completion，使用 Outbox、任务租约和可恢复 Worker。
...
- 访问安全：OIDC Authorization Code + PKCE、JWT Resource Server、RBAC、领域级数据隔离和变更审计。
- 文件安全：ClamAV `INSTREAM` 扫描接口、解析硬超时、API 与解析 Worker 进程隔离。
...
```mermaid
flowchart LR
    OIDC["OIDC / SSO"] --> UI["React 运营台 + PKCE"]
    UI --> API["Spring Boot API + JWT/RBAC"]
    API --> MINIO[("MinIO 原文件")]
    ...
    HYBRID --> MILVUS
    HYBRID --> PG
    HYBRID --> LLM["OpenAI-compatible Chat"]
    LLM --> API
```

## 技术栈

- 后端：Java 21、Spring Boot 4.1、Spring Security OAuth2 Resource Server、Spring Data JPA、Flyway、Micrometer
...
先配置远程 OpenAI-compatible Embedding、Chat Completion 和 OIDC 服务。为了节省本机磁盘，生产 Compose 不下载本地模型和身份服务。

```bash
cp .env.example .env
# 修改 .env 中的密码、模型地址和 OIDC 配置
...
`docker-compose.production.yml` 是资源受限电脑上的单节点生产拓扑验证基线，不等同于高可用生产集群。生产默认要求 OIDC、生成式模型和 ClamAV；若为本机轻量联调把 `FILE_SCAN_PROVIDER=noop`，liveness 仍可用，但 readiness 会保持 `OUT_OF_SERVICE`，不会误报生产就绪。
...
- Chat Completion 同样使用远程 API；失败时自动退回带来源的抽取式答案。
