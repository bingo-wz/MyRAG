...
Tika 与 Tesseract 已覆盖常见格式，但互联网不可信文件不应直接在 API 进程解析。对外生产必须把解析 Worker 放入隔离容器，限制 CPU、内存、执行时间和网络，并在前置阶段接入病毒扫描。
...
仓库已经实现格式/MIME/大小/字符上限、内容去重、任务租约和恢复；病毒扫描、硬超时沙箱、SSO/RBAC 与跨组件自动对账属于对外上线前的部署项。
