...
- `KnowledgeDocument`：知识正文、领域、来源、审核状态、索引版本、源文件哈希和唯一导入任务关联。
...
- Worker 在整个文件处理期间持续续租，并在每个长阶段边界校验所有权；进程崩溃后，兜底扫描器在租约过期后重新领取。
- 中断文件恢复到 `QUEUED` 后通过条件更新原子领取，避免两个 Worker 同时推进同一文件。
- 批次消息重复消费时，已完成或仍持有有效租约的任务不会再次执行；每个文件任务到知识文档的唯一关联防止索引后崩溃产生重复知识。
- 恢复次数超过上限时，批次和仍可恢复的文件会终止为 `FAILED`，避免毒任务无限占用资源。
